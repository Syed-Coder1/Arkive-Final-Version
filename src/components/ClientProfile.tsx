...
export function ClientProfile({ client, onBack }: ClientProfileProps) {
  const { getReceiptsByClient, getDocumentsByClient, employees } = useDatabase();
  ...
  const handleExportPaymentHistory = async () => {
    try {
      await exportService.exportClientPaymentHistory(
        { 
          ...client, 
          updatedAt: client.updatedAt ?? client.createdAt, 
          password: client.password ?? '', 
          type: client.type as "IRIS" | "SECP" | "PRA" | "Other"
        }, 
        clientReceipts
      );
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {/* Replace inline Add Task modal with TaskFormModal */}
      <TaskFormModal
        show={showTaskForm}
        onClose={() => {
          setShowTaskForm(false);
